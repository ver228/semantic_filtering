...
if __name__ == '__main__':
    ...
    bn = 'fmnist-bg-fix_l1smooth_20190212_185732_unet_adam_lr0.00032_wd0.0_batch32'
...
#                         is_v_flip = False)
    ...
    for ibatch, (xin, x_true) in tqdm.tqdm(enumerate(gen)):
        ...
        if is_plot:
            ...
            axs[1][0].imshow(x_pred, cmap='gray', vmin=0, vmax=1)
            axs[1][1].imshow(x_diff, cmap='gray', vmin=0, vmax=1)
            ...
            axs[0][0].set_title('Input (I)')
            axs[0][1].set_title('Ground Truth (GT)')
            axs[0][2].set_title(f'GT > {th}')
            
            axs[1][0].set_title('Prediction (P)')
            axs[1][1].set_title('Input - Prediction')
            axs[1][2].set_title(f'(I - P)>{th}, IoU={IoU:.3}')
    ...
    axs[1].imshow(img_bg1, cmap='gray')
    axs[2].imshow(img_fg2 + img_bg1, cmap='gray')
